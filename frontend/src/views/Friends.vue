    ...
    <div class="min-h-screen bg-gray-100 dark:bg-gray-900 pt-16">
        <div class="container mx-auto px-4 py-4 max-w-2xl">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-6">
                <h3 class="text-xl font-semibold mb-4 dark:text-gray-100">Find Friends</h3>
                <div class="flex gap-2 mb-4">
                    <input 
                        ...
                        class="flex-1 border dark:border-gray-600 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700 dark:text-gray-100 dark:placeholder-gray-400"
                    />
                    <button 
                        @click="handleSearch" 
                ...
                <div v-if="searchResults.length > 0" class="space-y-3">
                    <h4 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase">Results</h4>
                    <div v-for="user in searchResults" :key="user.id" class="flex items-center justify-between p-3 border dark:border-gray-600 rounded hover:bg-gray-50 dark:hover:bg-gray-700">
                        <div class="flex items-center gap-3">
                            <div class="bg-gray-200 w-8 h-8 rounded-full flex items-center justify-center text-gray-600 font-bold text-sm">
                                {{ user.username?.charAt(0).toUpperCase() }}
                            </div>
                            <span class="font-medium dark:text-gray-100">{{ user.username }}</span>
                        </div>
                        <button 
                ...
                <div v-else-if="searched && searchResults.length === 0" class="text-gray-500 dark:text-gray-400 text-sm italic">
            ...
            <!-- Pending Requests -->
            <div v-if="friendStore.pendingRequests.length > 0" class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-6">
                <h3 class="text-xl font-semibold mb-4 text-orange-600">Pending Requests</h3>
                <div class="space-y-4">
                    <div v-for="request in friendStore.pendingRequests" :key="request.id" class="flex justify-between items-center border-b dark:border-gray-700 pb-3 last:border-0 last:pb-0">
                        <div>
                            <span class="font-medium text-gray-800 dark:text-gray-100">{{ request.requesterName }}</span>
                            <span class="text-gray-500 dark:text-gray-400 text-sm ml-2">sent you a request</span>
                        </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                <h3 class="text-xl font-semibold mb-4 dark:text-gray-100">My Friends ({{ friendStore.friends.length }})</h3>
                ...
                <div v-else class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div v-for="friend in friendStore.friends" :key="friend.id" class="flex items-center justify-between p-3 border dark:border-gray-600 rounded hover:bg-gray-50 dark:hover:bg-gray-700">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <div class="font-medium text-gray-900 dark:text-gray-100">{{ friend.username }}</div>
                            </div>
                        </div>
                        <button 
